...
const ALPHA_CENTAURI_TOTAL_MASS =
    ALPHA_CENTAURI_A_MASS + ALPHA_CENTAURI_B_MASS;
...
```ts
    ...
    it("forwards barycenter overlay visibility to the underlying renderer", async () => {
        ...
        expect(() =>
            renderer.setBarycenterOverlayVisible(true),
        ).not.toThrow();
    });
```
...
```ts
        ...
        const validateOrbit = (
            orbit:
                | NonNullable<typeof system.star>["orbit"]
                | undefined,
            fieldPrefix: string,
        ) => {
            if (!orbit) return;
            ...
            if (
                ...
                warnings.push({
                    ...
                    message: "Eccentricity should be greater than or equal to 0 and less than 1",
                    severity: "warning",
                });
            }
