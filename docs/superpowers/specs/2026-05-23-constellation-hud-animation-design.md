...
- *Snap-in*: `120ms cubic-bezier(.2,.8,.2,1)` — reticles appearing, hover frames
- *HUD glide*: `420ms cubic-bezier(.25,.1,.25,1)` — panel and list reveals, camera tween easing
- *Energy flow*: `2.6s linear infinite` — selected constellation line dash
- *Scan-line drift*: `8s linear infinite` — full-screen overlay
